...
if platform.system() == "Linux":
    ...
    # Weiterhin erlaubt die KLasse eine Emulation des eKarrens mit dem RosMaster X3 Plus. 
    class eKarren:
        def __init__(self, device=DEV_ISAAC_SIM, debug=False):
            assert device == DEV_ISAAC_SIM
            self.clientAddr = ("127.0.0.1", 5006)            # 
            self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        ...
        def SetSpeed(self, vLinear, omega):
            ...
            data_bytes = cmd.tobytes()

            # Send via UDP
            self.sock.sendto(data_bytes, self.clientAddr)                
