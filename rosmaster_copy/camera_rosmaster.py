...
class Rosmaster_Camera(object):
    ...
    def __init__(self, video_id=0, width=640, height=480, debug=False):
        ...
        if self.__video_id == self.TYPE_DEPTH_CAMERA:
            self.__video = cv.VideoCapture('/dev/depth_cam')
            if self.__debug:
                print("video:camera_depth")
        elif self.__video_id == self.TYPE_USB_CAMERA:
            self.__video = cv.VideoCapture('/dev/usb_cam')
            if self.__debug:
        ...
        if self.__debug:
            print("---------Video:%s Init OK!------------" % str(self.__video_id))
    ...
    def reconnect(self):
        ...
        if not self.__state:
            if self.__debug:
                print("---------Video:%s Reconnect OK!------------" % str(self.__video_id))
            self.__state = True
            self.__config_camera()
        return True
